...
import { UsageSummaryResponse, formatCentsToDollars, calculateIndividualContribution } from '../services/api';
 ...
 */
export function updateStatusBar(data: UsageSummaryResponse): void {
 ...
 */
function createTooltip(data: UsageSummaryResponse): vscode.MarkdownString {
    ...
    if (data.teamUsage?.pooled?.enabled && data.teamUsage.pooled.used > 0) {
        const contribution = calculateIndividualContribution(individualUsed, data.teamUsage.pooled.used);
        tooltip.appendMarkdown(`**Your Contribution:** ${contribution.toFixed(1)}% of team pooled usage\n\n`);
    }
    ...
    if (data.teamUsage?.pooled?.enabled) {
        ...
        const remainingPercent = pooledLimit > 0 ? ((pooledRemaining / pooledLimit) * 100).toFixed(1) : '0.0';
        ...
        tooltip.appendMarkdown(`**Remaining:** ${formatCentsToDollars(pooledRemaining)} (${remainingPercent}%)\n\n`);
    }
